#!/usr/bin/env python3
"""
RL Controller для Webots - СБАЛАНСИРОВАННАЯ система наград с РАНДОМИЗАЦИЕЙ
"""
...
class BMWRLEnvironment(gym.Env):
    """
    RL среда с наградами и рандомизацией препятствий
    """
    ...
    def __init__(self, render_mode="human"):
        ...
        print("=" * 60)
        print("BMW X5 — RL СРЕДА с РАНДОМИЗАЦИЕЙ (v6.0)")
        print("=" * 60)
    ...
    def _calculate_reward(self, action):
        ...
        if current_speed > 1.0:
            angle_quality = 1.0 - (angle_to_target / np.pi)
            reward += angle_quality * 3.0
...
class ModelMetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        # Логирование метрик обучения (они уже автоматически логируются)
        # Но мы можем добавить свои
        ...
        if self.locals.get("dones") and any(self.locals["dones"]):
            ...
            for i, done in enumerate(dones):
                if done and infos[i]:
                    ...
                    # Базовая информация об эпизоде (уже есть в SB3)
                    if "episode" in info:
                        ep_info = info["episode"]
                        self.episode_rewards.append(ep_info["r"])
                        self.episode_lengths.append(ep_info["l"])

                        # Логируем средние за последние 100 эпизодов
...
def train():
    print("\n" + "=" * 60)
    print("ЗАПУСК ОБУЧЕНИЯ (С РАНДОМИЗАЦИЕЙ)")
    print("=" * 60)
    ...
    try:
        model.learn(
            ...
            tb_log_name="BMW_X5_RL_RANDOMIZED",
            reset_num_timesteps=False,
            callback=ModelMetricsCallback()  # Используем новый callback
        )

        final_path = "./models/bmw_rl_randomized_final"
        model.save(final_path)
        print(f"\nФинальная модель сохранена: {final_path}")

    except KeyboardInterrupt:
        print("\n\nОбучение прервано пользователем")
        interrupt_path = "./models/bmw_rl_randomized_interrupted"
...
def test(model_path="./models/bmw_rl_randomized_final.zip"):
    print("\n" + "=" * 60)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        if sys.argv[1] == "--test":
            model_path = sys.argv[2] if len(sys.argv) > 2 else "./models/bmw_rl_randomized_final.zip"
            test(model_path)
        else:
            print("Использование:")
