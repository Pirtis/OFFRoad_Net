...
class PPOAgent:
    ...
    def _init_networks(self, state_dim):
        if self.initialized:
            return
    ...
    def train(self):
        ...
        try:
            ...
            advantages, returns = self.compute_gae(rewards, old_values, dones, next_value.item())
            ...
            for _ in range(EPOCHS):
                ...
                for start in range(0, len(self.buffer), BATCH_SIZE):
                    ...
                    if len(batch_indices) == 0 or max(batch_indices) >= len(states):
                        continue
    ...
    def load(self, path):
        ...
        self._init_networks(checkpoint["state_dim"])
