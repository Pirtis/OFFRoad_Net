...
class PPOWaypointNavigator:
    def __init__(self):
        ...
        self.prev_distance_to_finish = float('inf')
    ...
    def plan_route(self):
        ...
        self.current_waypoints = self.path_planner.plan_path(start_pos, target_pos, self.obstacles)
        self.current_target_idx = 0
        self.prev_distance_to_target = float('inf')
        self.prev_distance_to_finish = float('inf')
    ...
    def get_distance_to_finish(self):
        pos = self.get_position()
        return math.sqrt((pos[0] - self.target_x)**2 + (pos[1] - self.target_y)**2)

    def update_nearest_waypoint(self, car_pos):
        if not self.current_waypoints:
            return
        min_dist = float('inf')
        nearest_idx = self.current_target_idx
        for i in range(self.current_target_idx, len(self.current_waypoints)):
            wp = self.current_waypoints[i]
            dist = math.sqrt((car_pos[0] - wp[0])**2 + (car_pos[1] - wp[1])**2)
            if dist < min_dist:
                min_dist = dist
                nearest_idx = i
        if nearest_idx > self.current_target_idx:
            print(f"SKIP: Jumped from waypoint {self.current_target_idx} to {nearest_idx}")
    ...
    def compute_reward(self, distance, angle_diff, min_front, min_left, min_right, speed):
        ...
        dist_to_finish = self.get_distance_to_finish()
        if self.prev_distance_to_finish != float('inf'):
            finish_progress = self.prev_distance_to_finish - dist_to_finish
            if finish_progress < -0.1:
                reward -= 0.5
        self.prev_distance_to_finish = dist_to_finish
    ...
    def step(self):
        ...
        self.update_nearest_waypoint(pos)
